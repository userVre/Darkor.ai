...
const WALL_ITEMS: DiscoverTile[] = WALL_COLOR_OPTIONS.map((option) => ({
  id: `wall-${option.id}`,
  title: option.title,
  subtitle: option.discoverSubtitle,
  spaceType: "Wall",
  style: option.discoverSubtitle.split("/").at(-1)?.trim() ?? option.title,
  service: "paint",
  image: option.image,
  startStep: "3",
  presetStyle: option.title,
  presetRoom: "Wall",
}));

const FLOOR_ITEMS: DiscoverTile[] = FLOOR_MATERIAL_OPTIONS.map((option) => ({
  id: `floor-${option.id}`,
  title: option.title,
  subtitle: option.discoverSubtitle,
  spaceType: "Floor",
...
export const DISCOVER_SECTIONS: DiscoverSection[] = [
  ...
  },
  {
    ...
    description: "Curated paint directions designed to open straight into the wall-color wizard.",
    items: WALL_ITEMS,
  },
  {
    id: "floor",
    title: "Floor Styles",
    description: "Material-first floor references that preload Step 3 in the floor restyle flow.",
    items: FLOOR_ITEMS,
  },
  {
    id: "garden",
