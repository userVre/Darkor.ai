...
export default function WorkspacePage() {
  ...
  const resultImages = useMemo(() => {
    if (!generatedImageUrl) return generatedFallbacks;
    return [generatedImageUrl, ...generatedFallbacks];
  }, [generatedImageUrl]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await saveGeneration({
        ...
        planUsed: userPlan === "pro" ? "Pro" : userPlan === "premium" ? "Premium" : "Ultra",
      });

      setWorkspaceState("success");
    } catch (error) {
      setWorkspaceState("idle");
      setFeatureToast(error instanceof Error ? error.message : "Could not generate image");
    }
  };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setFeatureToast(null), 2200);
  ...
  const renderDynamicSettings = () => {
    if (activeTab === "redesign") {
      return (
        <motion.div key="redesign" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }} className="space-y-5">
          <div className="space-y-2">
            <label className="text-xs font-medium uppercase tracking-wide text-zinc-400">Style Picker</label>
            <select value={style} onChange={(event) => setStyle(event.target.value)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30">
              {styles.map((item) => (
          ...
          <div className="space-y-2">
            ...
            <textarea value={customPrompt} onChange={(event) => setCustomPrompt(event.target.value)} placeholder="Describe your dream atmosphere, materials, colors, and lighting..." className="min-h-[120px] w-full resize-none rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition placeholder:text-zinc-500 focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30" />
          </div>
          {renderPromptChips()}
        </motion.div>
      );
    ...
    if (activeTab === "virtual-staging") {
      return (
        <motion.div key="virtual-staging" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }} className="space-y-5">
          <div className="space-y-2">
            <label className="text-xs font-medium uppercase tracking-wide text-zinc-400">Style Picker</label>
            <select value={style} onChange={(event) => setStyle(event.target.value)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30">
              {styles.map((item) => (
          ...
          <div className="space-y-2">
            ...
            <select value={roomType} onChange={(event) => setRoomType(event.target.value)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30">
              {roomTypes.map((item) => (
                <option key={item} value={item}>{item}</option>
              ))}
            </select>
          ...
          <div className="space-y-2">
            ...
            <select value={furnitureStyle} onChange={(event) => setFurnitureStyle(event.target.value)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30">
    ...
    return (
      <motion.div key="edit" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }} className="space-y-5">
        <div className="space-y-2">
          <label className="text-xs font-medium uppercase tracking-wide text-zinc-400">Style Picker</label>
          <select value={style} onChange={(event) => setStyle(event.target.value)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-fuchsia-500/70 focus:ring-2 focus:ring-fuchsia-500/30">
            {styles.map((item) => (
  ...
  const renderPremiumActionButton = (label: string, isPremium: boolean) => (
    <button
      ...
      type="button"
      onClick={() => {
        if (isPremium && isProPlan) handlePremiumFeature(label);
      }}
    ...
    <main className="h-[calc(100vh-72px)] overflow-hidden bg-zinc-950 text-zinc-100">
      <div className="flex h-full w-full">
          ...
          <div className="border-b border-white/5 px-6 py-5">
            ...
            <div className="mt-4 inline-flex rounded-full border border-white/10 bg-zinc-900/80 p-1">
              {(["pro", "premium", "ultra"] as const).map((plan) => (
                <button key={plan} type="button" onClick={() => setUserPlan(plan)} className={`rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-wide transition ${userPlan === plan ? "bg-white text-zinc-900" : "text-zinc-300 hover:bg-white/10 hover:text-white"}`}>
                  {plan}
                </button>
              ))}
            </div>
          ...
          <div className="flex-1 space-y-6 overflow-y-auto px-6 py-6">
            <div className="rounded-2xl border border-white/10 bg-zinc-900/60 p-1 backdrop-blur-xl">
              <div className="relative grid grid-cols-3 gap-1">
                {tabItems.map((tab) => (
                  <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`relative z-10 rounded-xl px-2 py-2.5 text-xs font-semibold transition ${activeTab === tab.id ? "text-white" : "text-zinc-400 hover:text-zinc-200"}`}>
                    {tab.label}
                  </button>
                ))}
                <motion.div layout layoutId="activeTabPill" className="absolute bottom-1 top-1 w-[calc(33.333%-0.333rem)] rounded-xl border border-fuchsia-400/30 bg-gradient-to-r from-fuchsia-500/20 to-cyan-500/20 shadow-[0_0_20px_rgba(217,70,239,0.24)]" animate={{ x: activeTab === "redesign" ? "0%" : activeTab === "virtual-staging" ? "102%" : "204%" }} transition={{ type: "spring", stiffness: 340, damping: 28 }} />
              </div>
            </div>
            ...
            <div className="border-t border-white/5 px-6 py-4">
              <div className="space-y-3 rounded-2xl border border-white/10 bg-zinc-900/70 p-4 backdrop-blur">
                ...
                <div className="space-y-2">
                  ...
                  <select value={cameraMotion} onChange={(event) => setCameraMotion(event.target.value as CameraMotion)} className="w-full rounded-xl border border-white/10 bg-zinc-900 px-3 py-2.5 text-sm text-zinc-100 outline-none transition focus:border-cyan-400/70 focus:ring-2 focus:ring-cyan-400/25">
                    {cameraMotions.map((motionType) => (
                      <option key={motionType} value={motionType}>{motionType}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-medium uppercase tracking-wide text-zinc-400">Speed</label>
                  <input type="range" min={0} max={100} value={cameraSpeed} onChange={(event) => setCameraSpeed(Number(event.target.value))} className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-zinc-800 accent-cyan-400" />
          ...
          <div className="border-t border-white/5 p-5">
            <div className="mb-3 space-y-3 rounded-2xl border border-white/10 bg-zinc-900/70 p-4 backdrop-blur">
              <div className="flex items-center justify-between gap-4">
                ...
                <button type="button" onClick={() => setIs3DMode((prev) => !prev)} className={`relative inline-flex h-7 w-12 items-center rounded-full border transition ${is3DMode ? "border-cyan-300/70 bg-cyan-500/70" : "border-white/20 bg-zinc-800"}`}>
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <button type="button" onClick={() => setIsTurbo((prev) => !prev)} className={`relative inline-flex h-7 w-12 items-center rounded-full border transition ${isTurbo ? "border-fuchsia-300/70 bg-fuchsia-500/70" : "border-white/20 bg-zinc-800"}`}>
                  <span className={`h-5 w-5 rounded-full bg-white transition ${isTurbo ? "translate-x-6" : "translate-x-1"}`} />
                </button>
              </div>
            </div>

            <button type="button" onClick={handleGenerate} className={`inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3.5 text-sm font-semibold text-white transition hover:brightness-110 ${isTurbo ? "animate-pulse bg-gradient-to-r from-fuchsia-600 to-purple-600 shadow-[0_14px_38px_rgba(168,85,247,0.4)]" : "bg-gradient-to-r from-zinc-800 to-zinc-700 shadow-[0_12px_26px_rgba(0,0,0,0.35)]"}`}>
              {is3DMode ? <ScanSearch className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
              {workspaceState === "loading" ? "Generating..." : is3DMode ? "Generate 3D Walkthrough" : "Generate Interior Renders"}
            </button>
          </div>
        </aside>
        ...
        <section className="relative flex flex-1 flex-col bg-[#0a0a0a] p-7">
          <AnimatePresence>
            {featureToast && (
              <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="absolute right-8 top-6 z-50 rounded-xl border border-amber-400/30 bg-zinc-900/95 px-4 py-2 text-sm text-amber-200 shadow-xl">
                {featureToast}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {workspaceState === "success" ? (
            ...
            <label
              ...
              onDrop={handleDrop}
              className={`group relative block h-full cursor-pointer overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/70 p-6 transition ${isDragging ? "border-fuchsia-500 bg-fuchsia-500/10 shadow-[0_0_0_1px_rgba(217,70,239,0.4),0_0_40px_rgba(217,70,239,0.2)]" : "hover:border-fuchsia-500 hover:bg-fuchsia-500/5"}`}
              onClick={() => {
            ...
            >
              {imagePreview ? (
                <>
                  ...
                  <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); fileInputRef.current?.click(); }} className="absolute left-10 top-10 z-30 rounded-lg border border-white/30 bg-black/65 px-3 py-1.5 text-xs font-medium text-white transition hover:border-fuchsia-500/70 hover:text-fuchsia-200">
                    Change Image
                  </button>

                  <button type="button" onClick={(event) => { event.preventDefault(); event.stopPropagation(); clearImagePreview(); }} className="absolute right-10 top-10 z-30 rounded-lg border border-white/30 bg-black/65 px-3 py-1.5 text-xs font-medium text-white transition hover:border-fuchsia-500/70 hover:text-fuchsia-200">
                    Remove Image
                  </button>
                </>
              ) : (
                <div className="flex h-full flex-col items-center justify-center gap-5 text-center">
                  <motion.div animate={{ y: [0, -6, 0], opacity: [0.8, 1, 0.8] }} transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }} className="rounded-3xl border border-fuchsia-500/30 bg-fuchsia-500/10 p-5 shadow-[0_0_30px_rgba(217,70,239,0.3)]">
              ...
              <div className="pointer-events-none absolute left-8 top-8 hidden items-center gap-2 rounded-full border border-white/10 bg-zinc-900/70 px-3 py-1.5 text-xs text-zinc-300 backdrop-blur md:inline-flex">
                ...
                {activeTab === "redesign" ? "Redesign Mode" : activeTab === "virtual-staging" ? "Virtual Staging Mode" : "Edit Mode"}
              </div>
            </label>
          )}
        </section>
