...
type ExteriorRedesignStepFourProps = {
  creditCount: number;
...
export function ExteriorRedesignStepFour({
  creditCount,
  palettes,
  selectedPaletteId,
  ...
  const titleLeft = 24;
  const paletteTitleTopGap = scaleValue(12, layoutScale);
        ...
        <View style={{ marginTop: paletteTitleTopGap, paddingHorizontal: paletteMargin }}>
          ...
          <View style={{ width: paletteGridWidth, alignSelf: "center" }}>
            {paletteRows.map((row, rowIndex) => (
              <View
                key={`exterior-palette-row-${rowIndex}`}
                style={{
                  flexDirection: "row",
